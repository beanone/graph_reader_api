...
from graph_reader_api.auth.dependencies import (
    API_KEY_HEADER,
...
@pytest.mark.asyncio
async def test_validate_api_key_local_valid(monkeypatch):
    ...
    monkeypatch.setattr("keylin.keylin_utils.hash_api_key", lambda k: key_hash)
    result = await validate_api_key_local(api_key, session)
...
@pytest.mark.asyncio
async def test_validate_api_key_local_invalid(monkeypatch):
    api_key = "invalidkey"
    session = AsyncMock()
    session.execute = AsyncMock(return_value=MagicMock(scalar_one_or_none=lambda: None))
    monkeypatch.setattr("keylin.keylin_utils.hash_api_key", lambda k: "bad_hash")
    with pytest.raises(HTTPException) as exc:
        await validate_api_key_local(api_key, session)
    assert exc.value.status_code == 401
    assert exc.value.detail == "Invalid API key"


@pytest.mark.asyncio
async def test_validate_api_key_local_expired(monkeypatch):
...
@pytest.mark.asyncio
async def test_validate_api_key_local_user_not_found(monkeypatch):
    api_key = "validkey"
    key_hash = "hashedkey"
    api_key_obj = MagicMock()
    api_key_obj.key_hash = key_hash
    ...
    monkeypatch.setattr("keylin.keylin_utils.hash_api_key", lambda k: key_hash)
    result = await validate_api_key_local(api_key, session)
    assert result["sub"] == api_key_obj.user_id
...
async def test_get_current_user_with_api_key(monkeypatch):
    ...
    # Patch validate_api_key_local to return expected_user_info
    monkeypatch.setattr(
        "graph_reader_api.auth.dependencies.validate_api_key_local",
        AsyncMock(return_value=expected_user_info),
    )
    # Build request with API key header
    headers = [(API_KEY_HEADER.lower().encode(), api_key.encode())]
    request = make_request(headers=headers)
    # session can be a dummy AsyncMock, since it's not used due to the patch
